...
class SubtitleParser:
    ...
    def clean_vtt_content(vtt_text: str) -> list[str]:
        ...
        for line in lines:
            ...
            line = SubtitleParser.clean_vtt_text(line)
    ...
    def parse_vtt_file(file_path: str) -> list[dict]:
        ...
        for block in blocks:
            ...
            if match:
                ...
                start = TimeConverter.vtt_time_to_seconds(start_str)
                end = TimeConverter.vtt_time_to_seconds(end_str)
                duration = max(0.0, end - start)
                text = " ".join(text_lines)
                text = SubtitleParser.clean_vtt_text(text)
    ...
    def parse_srt_content(content: str) -> list[dict]:
        ...
        for block in blocks:
            ...
            if match and text_lines:
                start = TimeConverter.srt_time_to_seconds(match.group(1), match.group(2), match.group(3), match.group(4))
                end   = TimeConverter.srt_time_to_seconds(match.group(5), match.group(6), match.group(7), match.group(8))
                duration = max(0.0, end - start)
                text = ' '.join(text_lines)
                text = SubtitleParser.clean_vtt_text(text)
                if text:
...
def time_to_seconds(time_str: str) -> float:
    return TimeConverter.time_to_seconds(time_str)

def vtt_time_to_seconds(t_str: str) -> float:
    return TimeConverter.vtt_time_to_seconds(t_str)

def srt_time_to_seconds(h: str, m: str, s: str, ms: str) -> float:
    return TimeConverter.srt_time_to_seconds(h, m, s, ms)

def clean_vtt_text(text: str) -> str:
    return SubtitleParser.clean_vtt_text(text)

def clean_vtt_content(vtt_text: str) -> list[str]:
    return SubtitleParser.clean_vtt_content(vtt_text)

def parse_vtt_file(file_path: str) -> list[dict]:
    return SubtitleParser.parse_vtt_file(file_path)

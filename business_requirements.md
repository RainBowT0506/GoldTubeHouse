...
## 10. List Mode (播放清單/多影片清單模式)

### 10.1 核心機制與時間軸映射
* 當載入 YouTube 播放清單（或多部影片網址）時，系統進入 **List Mode**。
* 系統將清單中的各別影片映射為 **章節 (Chapters)**，並以影片時長累加計算出一個 **「虛擬統一時間軸 (Virtual Unified Timeline)」**。
* 後續分段（Segment）、子段落（Subsegment）、範圍合併（Range Merging）及費用預估皆在該虛擬時間軸上進行，完全與 Chapter 模式相容。

### 10.2 影片分割與容錯限制
* **長影片分割**：若單部影片長度超過 30 分鐘，系統自動以 30 分鐘為基準進行智慧句尾對齊切分（Subsegments）。
* **無字幕容錯 (No Subtitle Fallback)**：當播放清單中某些影片無字幕或下載失敗時，系統會自動產生成對應的佔位卡片：`[(此影片無字幕文字)]`，確保整個清單載入流程不會中斷。
* **導出/複製時間還原**：複製整章 SRT 或段落內容時，系統會自動在背景扣除虛擬偏移量，還原為該影片的 Local 0 時間軸戳記。
